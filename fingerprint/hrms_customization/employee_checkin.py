...
class EmployeeCheckin(Document):
	...
	def fetch_shift(self):
		...
		if not self.attendance:
			...
			self.custom_shift_over_night_min = shift_actual_timings['shift_type']['custom_over_night'] # custom
			...
			if attendance_status == "Half Day" and (
				...
				frappe.db.set_value(
					...
					attendance.name,
					{
						...
						"working_hours": working_hours + int(over_night) / 60,
						"shift": shift,
						"late_entry": late_entry,
						"early_exit": early_exit,
						"in_time": in_time,
						"out_time": out_time,
						"custom_over_night_in_minutes": over_night,
						"custom_early_exit_in_minutes": custom_early_exit_in_minutes,
						"custom_late_entry_in_minutes": custom_late_entry_in_minutes,
					},
				)
			else:
...
def calculate_working_hours(logs, check_in_out_type, working_hours_calc_type):
	...
	if check_in_out_type == "Alternating entries as IN and OUT during the same shift":
		...
		if len(logs) >= 2:
			...
			# out_time = logs[-1].time + datetime.timedelta(hours=)
