...
class LeaveApplication(Document, PWANotificationsMixin):
	...
	def create_or_update_attendance(self, attendance_name, date):
		...
		if attendance_name:
			...
			doc.custom_hourly_leave = self.custom_hours,
...
def get_leaves_for_period(
	...
	for leave_entry in leave_entries:
		...
		if inclusive_period and leave_entry.transaction_type == "Leave Encashment":
			...
			if not frappe.db.get_value("Leave Type", leave_type, "include_holiday"):
				number_of_days = flt(leave_entry.leaves) - flt(
					get_holidays(employee, from_date, to_date, holiday_list=leave_entry.holiday_list)
				)
			...
			# 		half_day,
			# 		half_day_date,
			# 		leave_entry.leaves, # custom: number of hours EX:0.65
