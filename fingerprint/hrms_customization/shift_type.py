...
class ShiftType(Document):
	...
	def get_attendance(self, logs):
		...
		"""
		over_night = 0 # custom
		custom_late_entry_in_minutes = 0 # custom
		custom_early_exit_in_minutes = 0 # custom
		...
		if (
  		...
  		# custom code
		shift_over_night_min = int(logs[0]['custom_shift_over_night_min']) 
		if shift_over_night_min > 0: # over night shift 
			if len(logs) > 1:
				over_night = int(logs[-1].custom_over_night)
				if logs[0]['log_type'] == 'IN':
					custom_late_entry_in_minutes = round((logs[0]['time'] - logs[0]['shift_start']).total_seconds() / 60, 2)
				if logs[-1]['log_type'] == 'OUT':			
					custom_early_exit_in_minutes = round(((logs[-1]['shift_end'] - logs[-1]['time']).total_seconds() / 60) + (shift_over_night_min - over_night), 2)
			if len(logs) == 1:
				if logs[0]['log_type'] == 'IN':
					custom_late_entry_in_minutes = round((logs[0]['time'] - logs[0]['shift_start']).total_seconds() / 60, 2)
				elif logs[0]['log_type'] == 'OUT':
					custom_early_exit_in_minutes = round(((logs[0]['shift_end'] - logs[0]['time']).total_seconds() / 60) + (shift_over_night_min - over_night), 2)
		
			# if int(custom_early_exit_in_minutes) == 340:
			if (
				self.working_hours_threshold_for_absent
				and total_working_hours < self.working_hours_threshold_for_absent
			):
				return "Absent", total_working_hours, late_entry, early_exit, in_time, out_time, over_night, custom_early_exit_in_minutes, custom_late_entry_in_minutes

			if (
				self.working_hours_threshold_for_half_day
				and total_working_hours < self.working_hours_threshold_for_half_day
			):
				return "Half Day", total_working_hours, late_entry, early_exit, in_time, out_time, over_night, custom_early_exit_in_minutes, custom_late_entry_in_minutes

			return "Present", total_working_hours, late_entry, early_exit, in_time, out_time, over_night, custom_early_exit_in_minutes, custom_late_entry_in_minutes

		if len(logs) > 1:
			if logs[0]['log_type'] == 'IN':
				custom_late_entry_in_minutes = round((logs[0]['time'] - logs[0]['shift_start']).total_seconds() / 60, 2)
			if logs[-1]['log_type'] == 'OUT':			
				custom_early_exit_in_minutes = round((logs[0]['shift_end'] - logs[-1]['time']).total_seconds() / 60, 2)
		if len(logs) == 1:
			if logs[0]['log_type'] == 'IN':
				custom_late_entry_in_minutes = round((logs[0]['time'] - logs[0]['shift_start']).total_seconds() / 60, 2)
			elif logs[0]['log_type'] == 'OUT':
				custom_early_exit_in_minutes = round((logs[0]['shift_end'] - logs[0]['time']).total_seconds() / 60, 2)
